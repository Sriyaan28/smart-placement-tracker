...
import { MapPin, IndianRupee, Clock, Briefcase, Building2, ChevronLeft, CalendarDays, CheckCircle2, AlertCircle, Edit2, Trash2, X, BadgeCheck, Flag } from 'lucide-react';
import { useAuth } from '../../hooks/auth/useAuth';
import { useCompany } from '../../hooks/company/useCompany';
import { ReportModal } from '../../components/common/ReportModal';
import { AdminJobAnalytics } from '../../components/admin/AdminJobAnalytics';

export const JobDetails = ({ providedJobId }) => {
  const params = useParams();
  const jobId = providedJobId || params.jobId;
  ...
  return (
    <div className="w-full max-w-6xl mx-auto pb-20 animate-in fade-in duration-500 flex flex-col xl:flex-row gap-8 items-start px-4 xl:px-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        {user?.role === 'STUDENT' && (
          <button 
            ...
            className="flex items-center justify-center text-zinc-500 hover:text-red-400 transition-colors bg-zinc-900/50 hover:bg-zinc-900 border border-zinc-800/50 hover:border-red-500/20 w-10 h-10 rounded-lg group relative"
          >
            <Flag className="w-5 h-5" />
            <div className="absolute top-full right-0 mt-2 w-max px-3 py-1.5 bg-zinc-800 text-zinc-300 text-xs font-bold rounded-lg shadow-xl border border-zinc-700 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
              Report Job
            </div>
          </button>
        )}
        ...
        <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start justify-between">
          ...
          <div className="flex gap-6">
            ...
            <div className="flex flex-col justify-center">
              <h1 className="text-3xl md:text-4xl font-black text-white mb-2 tracking-tight pr-4 lg:pr-72">
                {job.title}
              </h1>
              <div 
              ...
              >
                ...
                {job.user?.isVerified && (
                  <div className="relative flex items-center group/badge">
                    ...
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-[200px] px-3 py-2 bg-zinc-800 text-zinc-300 text-xs font-medium rounded-lg shadow-xl border border-zinc-700 opacity-0 group-hover/badge:opacity-100 transition-opacity duration-200 pointer-events-none z-50 text-center">
      ...
      {/* Sticky Sidebar Admin Analytics for large screens */}
      {user?.role === 'ADMIN' && (
        <div className="hidden xl:block w-72 shrink-0 sticky top-8 z-10">
          <AdminJobAnalytics jobId={jobId} />
        </div>
      )}
